    ...
    public class AuthenticatedApiService
    {
        ...
        protected void CheckForError(IRestResponse response)
        {
            ...
            else if (!response.IsSuccessful)
            {
                ...
                {
                    message = "Authorization required: you must login for access.";
                }
                ...
                {
                    message = "An error occurred!!!!!!! That stinks.";
                }
                throw new HttpRequestException($"{message} Error code: {(int)response.StatusCode}");
            }
            return;
        }
    }
