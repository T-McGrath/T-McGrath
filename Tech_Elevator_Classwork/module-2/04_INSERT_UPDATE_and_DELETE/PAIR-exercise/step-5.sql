...
VALUES ('Ohiopyle State Park', 'Pennyslvania', '1965-01-01', 19052, 1000000, 'Ohiopyle State Park is a Pennsylvania state park on 19,052 acres in Dunbar, Henry Clay and Stewart Townships, Fayette County, Pennsylvania in the United States. The focal point of the park is the more than 14 miles of the Youghiogheny River Gorge that passes through the park.')
...
VALUES ((SELECT site_id FROM site WHERE site_number = 623), 'Wayne Family', GETDATE() + 10, GETDATE() + 20),
	((SELECT site_id FROM site WHERE site_number = 624), 'Wayne Family', GETDATE() + 10, GETDATE() + 20),
	((SELECT site_id FROM site WHERE site_number = 625), 'Wayne Family', GETDATE() + 10, GETDATE() + 20)

UPDATE reservation SET name = 'Parker Family'
WHERE reservation_id = 45;

UPDATE reservation SET name = 'Kent Family'
WHERE reservation_id = 46;
...
UPDATE reservation SET from_date = GETDATE(), to_date = GETDATE() + 7
...
WHERE name = 'Kent Family' AND site_id = (SELECT site_id FROM site WHERE site_number = 624)
