...
namespace TechElevator.Exercises.LogicalBranching
{
    ...
    public class ShippingTotal
    {
        ...
        {
            if (weightPounds > MaxWeightPounds)
            {
                return weightPounds * Over40PoundRate;
            }
            return weightPounds * UpTo40PoundRate;
        }
        ...
        public double CalculateShippingTotal(int weightPounds, bool hasDiscount)
        {
            ...
            if (weightPounds > MaxWeightPounds)
            {
                totalCost = weightPounds * Over40PoundRate;
            }
            else
            {
                totalCost = weightPounds * UpTo40PoundRate;
            }
            // Check for discount.
            if (hasDiscount)
            {
