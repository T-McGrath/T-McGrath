...
namespace TechElevator.Exercises.LogicalBranching
{
    ...
    public class HotelReservation
    {
        ...
        {
            double costWithParking;  
            if (numberOfNights < MinimumNightsForDiscountRate)
            {
                costWithParking = numberOfNights * DailyRate;
            }
            else
            {
                costWithParking = numberOfNights * DiscountRate;
            }
            // Add parking cost if necessary. Wanted to try the ternary operator...not sure if this is
            // considered good code practice or not.
            return includesParking ? costWithParking + numberOfNights * ParkingRate : costWithParking;
        }
        ...
        public double CalculateStayTotal(int numberOfNights, bool includesParking, bool includesLateCheckout)
        {
            ...
            if (numberOfNights < MinimumNightsForDiscountRate)
            {
                totalCost = numberOfNights * DailyRate;
            }
            else
            {
                totalCost = numberOfNights * DiscountRate;
            }
            // Add parking cost if necessary...I used the weird ternary operator for funsies.
            totalCost += includesParking ? numberOfNights * ParkingRate : 0;
            totalCost += includesLateCheckout ? LateCheckoutFee : 0;
            return totalCost;
