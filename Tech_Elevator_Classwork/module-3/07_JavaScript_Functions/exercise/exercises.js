 ...
 */
let makeNumber = (numString1, numString2) => {
    return Number.parseInt(numString1 + numString2);
}
/**
 ...
 */
const addHappy = (str) => "Happy " + str;
/**
 * Takes an array of words and a function that put 'Happy ' at the
 * beginning of each word.
 * 
 * Returns an array where all words begin with 'Happy '.
 * 
 * @param {string[]} strArray An array of words
 * @param {function} mapFunction Function that concatenates 'Happy ' to
 * the beginning of a string.
 * @returns An array of strings each beginning with 'Happy ';
 */
let makeHappy = (strArray, mapFunction = addHappy) => {
    return strArray.map(mapFunction);
}
...
/**
 * Take an address object array, and use every property within the array
 * to make full addresses.
 * 
 * Return an array of strings each containing a full address.
 * 
 * @param {addressObject[]} addressObjectArray an array of address objects
 * @returns an array of full addresses (as strings). 
 */
function getFullAddressesOfProperties(addressObjectArray) {
    return addressObjectArray.map((address) => {
 ...
 **/
let findLargest = (arrayOfStuff) => {
    max = arrayOfStuff[0];
    arrayOfStuff.forEach((numOrString) => {
        if (numOrString > max) {
            max = numOrString;
        }
    });
...
let getSumOfSubArrayValues = (twoDimArray) => {
    if (twoDimArray && twoDimArray.length > 0) {
        return twoDimArray.reduce((totalSum, curRow) => {
            return totalSum += curRow.reduce((rowSum, rowElement) => rowSum += rowElement, 0);
            }, 0);
    }
    return 0;
}
