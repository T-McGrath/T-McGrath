<template>
  <table id="tblUsers">
    ...
    <tbody>
      ...
      <tr v-for="user in filteredList" v-bind:key="user.emailAddress" v-bind:class="{'inactive' : user.status.toLowerCase() === 'inactive'}">
  ...
  computed: {
    filteredList() {
      const newList = this.users.filter((user) => {
        ...
        (user.status.toLowerCase() === this.search.status.toLowerCase() || this.search.status == "");
      });
      return newList;
    }
  }
