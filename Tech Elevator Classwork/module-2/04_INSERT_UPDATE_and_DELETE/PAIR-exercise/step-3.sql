...
-- select name and description of all parks order by established date in descending order (expected: 3 rows, starting with "Cuyahoga Valley")
...
SELECT COUNT(*) AS num_sites_with_utilities
FROM site
WHERE utilities = 1;
...
FROM reservation
WHERE from_date BETWEEN '2023-10-01' AND '2023-10-31';
...
SELECT from_date, COUNT(*) AS num_reservations
FROM reservation
GROUP BY from_date
